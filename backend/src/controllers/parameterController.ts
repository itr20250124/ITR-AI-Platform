...
export async function getProviderParameters(
  ...
  try {
    ...
    // 檢查提供商是否可用
    const isAvailable = AIServiceFactory.isProviderAvailable(
      provider,
 ...
 * 獲取參數預設值
 */
export async function getParameterDefaults(
  req: AuthenticatedRequest,
  res: Response
): Promise<void> {
  try {
...
export async function getParameterPresets(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    if (tag && typeof tag === 'string') {
      presets = globalParameterService
        .getProviderPresets(provider)
 ...
 * 創建自定義參數預設配置
 */
export async function createParameterPreset(
  req: AuthenticatedRequest,
  res: Response
): Promise<void> {
  try {

...
export class RateLimiter {
   ...
   * 執行單個規則
   */
  private async enforceRule(identifier: string, rule: RateLimitRule, now: number): Promise<void> {
    const key = `${identifier}:${rule.period}`;
    const records = this.requests.get(key) || [];

    // 清理過期記錄
    const validRecords = records.filter(record => now - record.timestamp < rule.period * 1000);
    ...
    if (currentRequests >= rule.requests) {
      ...
      const retryAfter = Math.ceil((oldestRecord.timestamp + rule.period * 1000 - now) / 1000);
  ...
  cleanup(): void {
    ...
    for (const [key, records] of this.requests.entries()) {
      ...
      const validRecords = records.filter(record => now - record.timestamp < period);

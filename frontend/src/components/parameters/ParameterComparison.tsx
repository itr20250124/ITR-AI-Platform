  ...
  const formatValue = (definition: ParameterDefinition, value: any) => {
    if (value === undefined || value === null) {
      return definition.defaultValue !== undefined ? 
        `${definition.defaultValue} (預設)` : '未設定';
    }
            ...
            {['creativity', 'consistency', 'cost', 'speed'].map((metric) => (
              <div key={metric} className="space-y-2">
                <h5 className="text-sm font-medium text-gray-700 capitalize">
                  {metric === 'creativity' ? '創意性' :
                   metric === 'consistency' ? '一致性' :
                   metric === 'cost' ? '成本' : '速度'}
                </h5>
                    ...
                    return (
                      <div key={set.id} className="flex items-center justify-between">
                        ...
                        <span className={`text-xs font-medium ${
                          value === '高' ? 'text-red-600' :
                          value === '中' ? 'text-yellow-600' : 'text-green-600'
                        }`}>
                          {value}
                        </span>
                      </div>
          ...
          <div className="text-sm text-gray-600">
            {(() => {
              const differentParams = definitions.filter(def => 
                getValueDifference(def, parameterSets.map(set => set.values[def.key]))
              );
              ...
              return `${differentParams.length} 個參數有差異: ${
                differentParams.map(p => p.key).join(', ')
              }`;
            })()}
          </div>
        </div>

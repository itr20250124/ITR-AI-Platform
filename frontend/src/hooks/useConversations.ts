...
export const useConversations = (options: UseConversationsOptions = {}): UseConversationsReturn => {
  const { autoLoad = true, limit = 20, orderBy = 'updatedAt', order = 'desc' } = options;
  ...
  const loadConversations = useCallback(
    async (refresh = false) => {
      try {
        ...
        setHasMore(
          response.conversations.length === limit &&
            offset + response.conversations.length < response.total
        );
      } catch (err) {
  ...
  const refresh = useCallback(async () => {
    ...
    setConversations([]);
    setHasMore(true);
    await Promise.all([loadConversations(true), loadStats()]);
